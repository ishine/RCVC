...
from utils.inference_utils import make_experiment_conversion_pair, pad_sequences, load_data, read_rhythm
...
def run_online_inference(hp, model, writer, iteration):
    ...
    experiment_dataset = make_experiment_conversion_pair(
        data_root, hp.dataset, hp.male, hp.female)
    ...
    with torch.no_grad():
        ...
        for i, conversion_pair in enumerate(experiment_dataset):
            ...
            # run model
            outputs_A2A = model.inference(
                mel_A, mel_len_A, rhythm_A, mel_A, mel_len_A, rhythm_A) 
            
            outputs_A2B = model.inference(
